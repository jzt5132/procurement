...
a_list = seq(50, 500, 50)
jump_freq = list()
jump_freq_equil = list()
rv = list()
max_jump_size = list()
...
for (loop in 1:length(supply_seed)){
  ...
  a = a_list[iii] #parameter in search cost sc= a * EZ^2 * s_k
  b = 50
  K = 1:K_
  I = 1:I_
...
jump_freq_equil[[iii]] = dim(oo1)[1]


realized_vol = sqrt(sum(diff(log(price_close))^2))
rv[[iii]] = realized_vol
max_jump_size[[iii]] = max(jump_metric3) 
}

cbind(unlist(jump_freq), unlist(jump_freq_equil), unlist(max_jump_size), unlist(rv))
...
# hist(supply_seed)
supply_seed = as.integer(supply_seed); supply_seed1 = supply_seed   # rbinom must take integer as parameter
bid_price_loop = list()
...
for (loop in 1:length(supply_seed)){
  ...
  D_ = rnorm(I_, 4.4, 1)
  c = rnorm(I_, 100,  4)
  S_ = rnorm(K_, supply_seed[loop], 3); if(loop == 1){S1 = S_}
  D = D_
  S = S_
  Z = sum(D) - sum(S)
...
#  ggtitle("Transaction Prices in Type 1 Trading Sessions (10 Suppliers & 100 Buyers)")+
  ...
  scale_y_continuous(labels = c("100", "125", "150", "175", "200"),
...
ggsave("Comp_Effect_Dist_Bid_0.pdf", width = 8, height = 6, units = "in")
# 
...
ggplot(candle_chart_df)+
  ...
  xlab("Trading Session")+
  ylab("Initial Excess Demand at the Beginning of each Trading Session")+
...
candle_chart_df_0 = candle_chart_df
# values=c("#0066FF", "#0033CC", "#33FFFF", "#17FF00", "#99CC00", "#FFCC66", "#FF9933", "#FF99FF", "#9933FF","#FF0000","#560000")

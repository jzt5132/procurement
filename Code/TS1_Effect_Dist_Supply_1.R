...
random_gumble = rgumbel(100, loc = 5, scale = 5)
# hist(random_gumble)
supply_seed = 54 - random_gumble
# hist(supply_seed)
...
for (loop in 1:length(supply_seed)){
  ...
  a = 200 #parameter in search cost sc= a * EZ^2 * s_k
  K = 1:K_
  I = 1:I_
  ...
  D_ = rnorm(I_, 4.4, 1)
  c = rnorm(I_, 100,  4)
  S_7 = rnorm(6, 0.825 * supply_seed[loop], 2)
  S_2 = rnorm(3, 1.30* supply_seed[loop], 5)
  S_1 = candle_chart_df_0$total_supply[loop] - sum(S_2) - sum(S_7)
  S_ = c(S_1, S_2, S_7); if(loop == 1){S2 = S_}
  D = D_
  S = S_
  Z = sum(D) - sum(S)
  ...
  c(1:K_)
  bid_= sapply(1:I_, function(i){optimal_bid_fn(buyers_visit_order, i , S, a)})
  ...
  ################################################
  # #acceptance rule 1
  # B_1 = function(b_ik, b_k_est, WC_k_est){
  #    return(b_ik >= b_k_est - WC_k_est)
  #   }
...
jump_metric2 = (price_max[,2] - price_equil)/price_equil * 100
jump_metric3 = (price_close - price_equil)/price_equil * 100
total_supply = unlist(total_supply_loop)
...
xlab("Trading Session")+
 ...
 # ggtitle("Transaction Prices in Type 1 Trading Sessions (10 Suppliers & 100 Buyers)")+
...
#  scale_y_continuous(labels = c("100", "120", "140"),
#                     breaks = c(100, 120, 140))+
...
ggsave("Comp_Effect_Dist_Bid_1.pdf", width = 8, height = 6, units = "in")
...
ggplot(candle_chart_df)+
  ...
  scale_x_discrete(labels= seq(0, length(supply_seed) - 10, 10),
                   breaks = seq(0, length(supply_seed) - 10, 10))+
  scale_color_manual(name = "Legend",
                     labels = c("Number of Encounters", "Number of Transactions"),
                     values = c("red", "blue"))+
  theme(axis.text.x = black.plain.angle30.size1.text,
        panel.background = element_rect(fill = "white"),
        panel.border = element_rect(fill = NA, colour = "grey", size = 0.8),
...
ggplot(candle_chart_df)+
  ...
  xlab("Trading Session")+
  ylab("Jump Size (%)")+
 # ggtitle("Jump Size across Trading Sessions (Maximal Bid Price)")+
  scale_x_discrete(labels= seq(0, length(supply_seed) - 10, 10),
                   breaks = seq(0, length(supply_seed) - 10, 10))+
  # scale_y_continuous(labels = c("20%", "40%", "60%"),
  #                    breaks = c(20, 40, 60))+
...
candle_chart_df_1 = candle_chart_df
# values=c("#0066FF", "#0033CC", "#33FFFF", "#17FF00", "#99CC00", "#FFCC66", "#FF9933", "#FF99FF", "#9933FF","#FF0000","#560000")

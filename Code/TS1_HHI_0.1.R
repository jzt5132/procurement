...
for (loop in 1:N_){
  ...
  #S_ = c(rep(0.5 * sum(S_org)/K_, 8), sum(S_org)/K_, 5 * sum(S_org)/K_)
  #S_ = c(rep(0.5 * sum(S_org)/K_, 8), 3 * sum(S_org)/K_, 3 * sum(S_org)/K_)
  #S_ = c(rep(0.01 * sum(S_org)/K_, 8), sum(S_org)/K_, 8.92 * sum(S_org)/K_)
  #S_ = c(rep(0.25 * sum(S_org)/K_, 8), sum(S_org)/K_, 7 * sum(S_org)/K_)
  #S_ = c(rep(0.5 * sum(S_org)/K_, 9), 5.5 * sum(S_org)/K_)
  S_ = c(rep(0.45 * sum(S_org)/K_, 9), 5.95 * sum(S_org)/K_)
  S_ = abs(S_)
